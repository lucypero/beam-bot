...
function command_post_rules(message: Message) {
    const rules_de = [
        ...
        "Kein nervendes Nachfragen nach gebannten Personen. Wenn eine Person gebannt ist, kann diese Person unter https://bit.ly/BeamDCUnban einen Entbannungsantrag stellen. (bei DC Bann). Und per Ticket bei mute um die Angelegenheit klären.",
    ...
    const rules_en = [
        ...
        "No repetitive questions about the bans of other members or yourself, appeals can be made at https://bit.ly/BeamDCUnban, only if it is a ban from Beam's discord. For any other punishment a ticket must be opened.",
...
async function post_audit_log(channel: TextChannel, knex_instance: Knex) {
    ...
    console.log("posting audit log in guild: ", channel.guild.id);
    ...
    const fetch_last_few_logs = async function (
        ...
        try {
            ...
            console.log("error fetching audit logs of guild: ", channel.guild!.id);
            console.log(e);
